...
-Download the .xlsx table and the .nb or .wl files and put them into a folder _x_.
...
-On the _restriccionCantidad = Thread[Total[varsSubconjunto] <= 3]_ field, you can change the value to another integer number, this will be the maximum of different products for _THE ENERGYNATOR_ to choose among the table.
...
Each price value on _preciosSubconjunto_ is assigned by _varsSubconjunto_ to an integer variable _x_, with _x_ going from 0 to _n_, on array format. This is the general modeling of the optimizable linear function:
...
_restriccionNeg_ and _restriccionEntera_ define _solucion_ as a natural (non-negative integer) number set, while _restriccionCantidad_ defines the maximum of the natural number _x_, that's the domain of the _solucion_ lineal sum function starting from 0. _restriccionCantidad_ can be manually modified to set the maximum of different catalog products to buy.
...
This format is then cleaned and expressed to vector form _{x_1,x_2...,x_n}_ within the _Flatten_ built-in Mathematica function by the _indicesActivos_ variable. Now we have the _solucion_ solution vector.
